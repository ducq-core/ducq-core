...
void log_sent_to_monitor_route(void **state) {
	// arrange
	ducq_reactor *reactor = ducq_reactor_new();
	ducq_reactor_allow_log_route(reactor, true);
	...
	ducq_reactor_subscribe(reactor, ducq1, DUCQ_LOG_ROUTE);
...
void log_dont_sent_to_monitor_route_if_not_set(void **state) {
	// arrange
	ducq_reactor *reactor = ducq_reactor_new();
	ducq_reactor_allow_log_route(reactor, false);
